...
namespace CyberTech.Application.Services
{
    public class PlayerService : IPlayerService
    {
        ...
        {
            var player = await _playerRepository.GetAsync(id, CancellationToken.None) ?? throw new Exception($"Запись с идентфикатором {id} не найдена");
        ...
        public async Task<string> GetPlayerImageIdAsync(Guid id)
        {
            var player = await _playerRepository.GetAsync(id, CancellationToken.None) ?? throw new Exception($"Запись с идентфикатором {id} не найдена");
