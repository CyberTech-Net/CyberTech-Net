...
namespace CyberTech.Application.Services
{
    public class GameTypeService : IGameTypeService
    {
        ...
        {
            var gameType = await _gameTypeRepository.GetAsync(id, CancellationToken.None) ?? throw new Exception($"Запись с идентфикатором {id} не найдена");            
        ...
        public async Task<string> GetGameTypeImageIdAsync(Guid id)
        {
            var gameType = await _gameTypeRepository.GetAsync(id, CancellationToken.None) ?? throw new Exception($"Запись с идентфикатором {id} не найдена");
            return gameType.ImageId;
        }
    }
}
