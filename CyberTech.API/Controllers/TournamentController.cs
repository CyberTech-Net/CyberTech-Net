...
namespace CyberTech.Api.Controllers
{
    ...
    public class TournamentController(ITournamentService tournamentService, IMapper mapper, IValidator<CreatingTournamentModel> createValidator, IValidator<UpdatingTournamentModel> updateValidator) : ControllerBase
    {
        ...
        {
            var response = _mapper.Map<List<TournamentModel>>(await _service.GetPagedAsync(page, itemsPerPage, cancellationToken));
            return Ok(response);
        }
        ...
        /// <returns></returns>
        [HttpPut("{id:guid}")]
        public async Task<IActionResult> EditAsync(Guid id, UpdatingTournamentModel updatingTournamentModel, CancellationToken cancellationToken = default)
        {
